...
When Convoy runs inside Zellij (including over SSH), `o` and `i` open OpenCode in a focused sibling pane rather than a macOS window. Exit OpenCode (for example with `Ctrl+C`) to close that pane and return to Convoy; Zellij's normal focus shortcut returns to Convoy without closing it. Set `CONVOY_TERMINAL=zellij`, `ghostty`, or `terminal` to override automatic backend selection; `zellij` requires an active Zellij session.
