...
4. **Squash** — Convoy collapses its own machine commits into one conventional commit; your own commits on the branch survive. That commit's message is composed by a model-backed writer (with a deterministic fallback when no model answers): the scope is always the single touched capability, the subject is a readable imperative line, and the change id is named in the body. The checklist names each squash sub-phase as it happens — composing the message, waiting for your review, creating the commit — and the running indicator keeps animating while the writer works, even when nothing new comes back. In a terminal you confirm, edit, or cancel the message before it lands; `--message` overrides it exactly and skips composition.
...
│   ├── finish.ts        # close's squash primitives (the retired finish command is gone)
