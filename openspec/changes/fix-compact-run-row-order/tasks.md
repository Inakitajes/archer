...
- [ ] 3.2 Run the full suite and typecheck (`bun test` and `bun run typecheck`) and confirm no regressions; sanity-check by hand that a goal run's dashboard shows goal invocation groups above the pending `Compact run` row
  - Automated portions done and green: full `bun test` (2928 pass, 0 fail), `bun run typecheck`, coverage gate. A rendering test (`test/tui.test.ts`, "a pending Compact run row renders below goal invocation rows…") asserts the goal-rows-above-pending-row invariant, but the by-hand dashboard check against a real goal run is still outstanding — perform it before archiving this change.
