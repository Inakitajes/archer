...
The Home launcher SHALL display a left-aligned three-line `CONVOY` wordmark in one uniform neutral tone. To its right, the masthead SHALL show the full build line (version, commit, platform) right-aligned on the first row and the project path right-aligned on the second row. The launcher SHALL reserve one blank row above the wordmark and one blank row between the masthead and the body content. Compact widths SHALL fall back to a text wordmark with the version right-aligned and a labeled project row below, without overflowing the terminal width. The launcher MUST NOT render a footer.

#### Scenario: Wide masthead
- **WHEN** Home opens with enough width for the full wordmark, version line, and project path
- **THEN** the wordmark is left-aligned, the full version line is right-aligned on the first masthead row, and the project path is right-aligned on the second masthead row

#### Scenario: Compact masthead
- **WHEN** the terminal cannot fit the block wordmark beside the full version line
- **THEN** Home shows a text `CONVOY` wordmark with the version right-aligned and a labeled `project  <path>` row below it
