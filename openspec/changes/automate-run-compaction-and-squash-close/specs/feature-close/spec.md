...
**Reason**: The old contract preserves operator commits and allows fast-forward/ordinary merge shapes, which is incompatible with an always-one-commit squash landing that leaves feature history intact.
**Migration**: Use the replacement requirement `Close always squash-lands the complete feature`. Close retains its entry point but always lands one regular commit for a non-empty feature result, and cleanup requires verified landing evidence instead of ordinary merged ancestry.
