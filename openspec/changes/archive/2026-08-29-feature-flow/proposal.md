...
- **Global `/convoy-spin` OpenCode command, opt-in via `convoy opencode install`.** A thin prompt wrapper that tells the agent in the current session to run `convoy spin` and relay the next steps; no inference, no pipeline guessing. Installing is an explicit operator decision — no path through spin or config save writes into the global config as a side effect — and the `convoy-` name avoids colliding with operator-owned commands. A minimal renacer of the removed `opencode-install`: one markdown file in `~/.config/opencode/commands/`, nothing per-repo.
...
- `feature-spin`: deterministic worktree creation from an uncommitted change — conventional-prefix inference, branch/worktree naming, moving the change files, the `/move` handoff, and the opt-in `/convoy-spin` wrapper.
